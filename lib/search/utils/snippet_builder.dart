...
class SnippetBuilder {
  ...
  static List<_SearchToken> _collectSearchTokens(String plainText) {
    ...
    for (final match
        // מפצלים על מקף עברי (־ U+05BE) ופסק (׀ U+05C0) לסימטריה עם sanitizeQuery.
        // כולל ' לועזי לסימטריה עם ".
        in RegExp(r'[א-תA-Za-z0-9"\u0027״׳\u0591-\u05BD\u05BF\u05C1-\u05C7]+').allMatches(plainText)) {
  ...
  static String _termToRegexPattern(String term) {
    ...
    for (var i = 0; i < chars.length; i++) {
      final ch = chars[i];
      // sanitizeQuery \u05DE\u05DE\u05D9\u05E8 \u05F4 \u05DC-" \u05D5-\u05F3 \u05DC-' \u05D1\u05DE\u05D5\u05E0\u05D7, \u05D0\u05D1\u05DC \u05D4\u05D8\u05E7\u05E1\u05D8 \u05D4\u05DE\u05D5\u05E6\u05D2 \u05E0\u05E9\u05D0\u05E8 \u05E2\u05DD \u05D4\u05E6\u05D5\u05E8\u05D4
      // \u05D4\u05E2\u05D1\u05E8\u05D9\u05EA. \u05DC\u05DB\u05DF " \u05D1\u05DE\u05D5\u05E0\u05D7 \u05DE\u05EA\u05D0\u05D9\u05DD \u05D2\u05DD \u05DC-\u05F4 \u05D1\u05D8\u05E7\u05E1\u05D8, \u05D5-' \u05DE\u05EA\u05D0\u05D9\u05DD \u05D2\u05DD \u05DC-\u05F3.
      if (ch == '"') {
        buffer.write('["\u05F4]');
      } else if (ch == "'") {
        buffer.write("['\u05F3]");
