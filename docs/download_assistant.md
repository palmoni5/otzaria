...
אשף ההורדות (`Otzaria-Download-Assistant-win.exe`) הוא כלי עזר בעברית שמסייע
  ...
  "components": [                // ממוין לפי installOrder ואז id
    {
      ...
      "name": "אוצריא לחלונות",              // עברית — מוצג למשתמש
...
```bash
ISCC installer\download_assistant.iss     # -> installer\Otzaria-Download-Assistant-win.exe
```
...
מקטעי `[Files]`, `[Icons]`, `[Registry]`, `[Dirs]` או `[INI]`. הוא גם
**חסר-גרסה**: `tool/version/update_version` אינו נוגע בו, והתג נקרא בזמן ריצה.
העמוד הראשון אומר את זה במפורש: "כלי זה אינו מתקין את אוצריא."

### מסלול הריצה

1. `InitializeSetup` פונה ל-`api.github.com/repos/Otzaria/otzaria/releases/latest`,
   קורא `tag_name`, מאתר ברשימת הנכסים את זה שמסתיים ב-`release-manifest.json`,
   ומוריד אותו לפי **אותו תג**. התג **ננעל לכל הריצה** — release שמתעדכן באמצע
   הורדה היה מערבב קבצים משתי גרסאות.
2. אם המניפסט חסר, פגום, או בגרסת סכמה אחרת — אין הודעת קריסה: מוצגת הודעה
   בעברית, והנסיגה היחידה שמוצעת היא **פתיחת עמוד ההורדות בדפדפן**. האשף
   לעולם אינו מוריד רכיב שאין לו ממנו hash.
3. העמודים: מצב (מחשב זה / מחשב אחר) ← ארכיטקטורת היעד (רק למחשב אחר) ←
...
קיים, ושהפלט נקרא בדיוק `Otzaria-Download-Assistant-win` (בלי `windows` ובלי
`full`, שהיו הופכים אותו למועמד לעדכון מתוך התוכנה).
...
(`Otzaria-Download-Assistant-win.exe`) אינו נתפס בשום תבנית — הן פותחות ב-
`^otzaria-` באותיות קטנות ומסתיימות ב-`-windows.exe`, לא `-win.exe`.
